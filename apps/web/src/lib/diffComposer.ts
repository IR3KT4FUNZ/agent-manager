...
import type { AskAgentRequest, AskAgentResult, DiffSelectionContext, FileDiff, PrSide } from "@agent-manager/shared";
import { askAgent } from "./api";

export interface SelectionAnchor { side: PrSide; line: number; startLine?: number }
...
export function captureDiffContext(diff: FileDiff, anchor: SelectionAnchor): DiffSelectionContext {
  ...
  const lines = diff.hunks.flatMap(hunk => hunk.lines).flatMap(item => {
    const line = side === "old" ? item.oldLine : item.newLine;
    return line !== null && line >= startLine && line <= anchor.line ? [{ line, text: item.text }] : [];
  });
  return {
    path: side === "old" ? diff.oldPath ?? diff.path : diff.path,
    side, startLine, endLine: anchor.line, lines,
    reviewHeadSha: diff.reviewHeadSha,
    base: diff.base, currentVersion: diff.currentVersion, selectedAt: new Date().toISOString(),
  };
}

export function contextPreview(context: DiffSelectionContext): string {
  let previous = context.startLine - 1;
  return context.lines.flatMap(item => {
    const gap = item.line > previous + 1 ? ["… intervening lines not displayed …"] : [];
    previous = item.line;
    return [...gap, `${item.line}: ${item.text}`];
  }).join("\n");
}
...
export class DiffComposerStore {
  ...
  private fileKey(sessionId: string, path: string) { return JSON.stringify([sessionId, path]); }
  get(sessionId: string, path: string) { return this.drafts.get(this.active.get(this.fileKey(sessionId, path)) ?? ""); }
  focusKey(sessionId: string) { return this.focus.get(sessionId) ?? 0; }
  open(sessionId: string, diff: FileDiff, anchor: SelectionAnchor) {
    ...
    if (!this.drafts.has(id)) this.drafts.set(id, {
      id, sessionId, path: diff.path, anchor,
      context: captureDiffContext(diff, anchor), destination: "agent", text: "",
      requestId: crypto.randomUUID(), pending: false,
    });
  ...
  update(id: string, patch: Partial<Pick<ComposerDraft, "text" | "destination">>) {
    ...
    this.drafts.set(id, { ...draft, ...patch, error: undefined,
      requestId: patch.text !== undefined && patch.text !== draft.text ? crypto.randomUUID() : draft.requestId });
    this.emit();
  }
  cancel(id: string) {
    const draft = this.drafts.get(id);
  ...
  async send(id: string, deliver: (sessionId: string, request: AskAgentRequest) => Promise<AskAgentResult> = askAgent) {
    const draft = this.drafts.get(id);
    ...
    this.emit();
    try {
      await deliver(draft.sessionId, { requestId: draft.requestId, question: draft.text, context: draft.context });
      this.focus.set(draft.sessionId, this.focusKey(draft.sessionId) + 1);
      this.remove(draft);
    } catch (error) {
      this.drafts.set(id, { ...draft, pending: false, error: error instanceof Error ? error.message : String(error) });
      this.emit();
    }
  }
}
