...
export function reviewRoutes(manager: Pick<SessionManager, "get">) {
  ...
  app.use(
    "/:id/*",
    bodyLimit({
      maxSize: MAX_QUESTION_BYTES,
      onError: (c) => c.json({ error: "Review request exceeds 64 KiB." }, 413),
    }),
  );
  ...
  app.onError((error, c) =>
    c.json(
      { error: error.message },
      error instanceof QuestionError || error instanceof SourceError
        ? error.status
        : 400,
    ),
  );
  app.get("/:id/walkthrough", async (c) =>
    c.json(await c.get("session").walkthrough.state()),
  );
  app.post("/:id/walkthrough", async (c) => {
    const body = await c.req.json();
    if (typeof body?.requestId !== "string")
      throw new QuestionError("Invalid walkthrough request.");
    return c.json(c.get("session").walkthrough.generate(body.requestId), 202);
  });
  app.get("/:id/walkthrough/reference/:nodeId", async (c) =>
    c.json(
      await c
        .get("session")
        .walkthrough.resolveReference(
          c.req.query("version") ?? "",
          c.req.param("nodeId"),
        ),
    ),
  );
  app.get("/:id/walkthrough/source/:nodeId", async (c) =>
    c.json(
      await c
        .get("session")
        .walkthrough.source(
  ...
  app.post("/:id/review/messages", async (c) =>
    c.json(c.get("session").review.submit(await c.req.json()), 202),
  );
