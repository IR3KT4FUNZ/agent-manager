...
- **Panels are resized by dragging the divider on their left edge** (`PanelBoard.tsx` + `lib/panelWidths.ts`): each panel carries a `defaultWidth`, the visually-last panel is the flexible one that absorbs the change, and widths are clamped to `MIN_PANEL_WIDTH` and persisted in `localStorage` (`agent-manager.panel-widths`). Fixed panels use `flex: 0 1 <width>px` with an explicit `min-width` so a narrow window shrinks them instead of overflowing the board.
